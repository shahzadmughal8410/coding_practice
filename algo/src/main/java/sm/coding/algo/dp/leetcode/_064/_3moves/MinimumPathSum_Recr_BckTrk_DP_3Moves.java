 ...
 * 
 */
package sm.coding.algo.dp.leetcode._064._3moves;
 ...
 *
 */
public class MinimumPathSum_Recr_BckTrk_DP_3Moves {
...
Your runtime beats 94.85 % of java submissions.
    ...
    public static int minPathSum_DP(int[][] grid) {
    		...
    		for(int i =1; i<grid.length;i++) {
    			for(int j =1; j<grid[0].length;j++) {
    				dp[i][j] = Math.min(dp[i-1][j-1], Math.min(dp[i-1][j], dp[i][j-1]) ) + grid[i][j];
    			}
