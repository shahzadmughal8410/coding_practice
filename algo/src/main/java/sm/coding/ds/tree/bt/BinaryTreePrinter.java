...
public class BinaryTreePrinter {
	...
	public static <T extends Comparable<?>> void preetyPrintTree(TreeNode root) {
		int maxLevel = BinaryTreePrinter.maxLevel(root);

		preetyPrintTreeInternal(Collections.singletonList(root), 1, maxLevel);
	}

	private static <T extends Comparable<?>> void preetyPrintTreeInternal(List<TreeNode> nodes, int level,
			int maxLevel) {
		...
		List<TreeNode> newGenericNodes = new ArrayList<TreeNode>();
		for (TreeNode GenericNode : nodes) {
			if (GenericNode != null) {
				System.out.print(GenericNode.val);
				newGenericNodes.add(GenericNode.left);
				newGenericNodes.add(GenericNode.right);
			} else {
				newGenericNodes.add(null);
				newGenericNodes.add(null);
				System.out.print(" ");
			}
		...
		preetyPrintTreeInternal(newGenericNodes, level + 1, maxLevel);
	}
